...
public class ValidationItemControllerV1 {
    ...
    public String addItem(@ModelAttribute Item item, RedirectAttributes redirectAttributes, Model model) {
        ...
        //실패 로직
        Map<String, String> errors = new HashMap<>(); // 어떤 검증에서 오류가 발생했는지 정보를 담아준다.
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            ...
            if (rstPrice < 1000) {
                errors.put("globalError", "가격 x 수량의 합은 10000원 이상이어야 합니다. 현재 값 = " + rstPrice);
            }
        }

        //검증에 실패하면 다시 입력 폼으로
        if (!errors.isEmpty()) { //이중 부정
